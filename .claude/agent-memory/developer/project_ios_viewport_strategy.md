...
- `html`: OWNS the top + side safe-area insets and CARRIES the sky.
  `height:100%; min-height: calc(100% + env(safe-area-inset-top)); padding: env(safe-area-inset-top) env(safe-area-inset-right) 0 env(safe-area-inset-left); overflow:hidden;` plus the sky as a `background-attachment: fixed` image (`var(--color-surface-solid) url("/assets/weather-hero.png") center top / cover no-repeat`). The `min-height` calc prevents the bottom white bar; the bottom inset is owned by `.decision-sheet`.
